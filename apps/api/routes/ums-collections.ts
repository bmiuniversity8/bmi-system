...
export async function handleListStudyCenters(request: Request, env: Env): Promise<Response> {
  ...
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM study_centers sc ${where}`).bind(...bindings).first<{c:number}>())?.c) || 0;
...
export async function handleListLibraryBooks(request: Request, env: Env): Promise<Response> {
  ...
  const where = `WHERE ${filters.join(' AND ')}`;
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM library_books ${where}`).bind(...bindings).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM library_books ${where} ORDER BY title LIMIT ? OFFSET ?`).bind(...bindings, perPage, offset).all();
  // Map to frontend LibraryItem interface
  type BookRow = Record<string, unknown>;
  const items = results.map((b: BookRow) => ({
    id: b.id as string, title: b.title as string, author: b.author as string, category: b.category as string, type: b.type as string,
    status: b.status as string, year: b.year as number, description: b.description as string, downloadUrl: b.download_url as string,
    location: b.location as string, isbn: b.isbn as string, created: b.created_at as string, updated: b.updated_at as string
  }));
...
export async function handleListHostels(request: Request, env: Env): Promise<Response> {
  const url = new URL(request.url);
  const { page, perPage, offset } = paginate(url);
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM hostels`).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM hostels ORDER BY name LIMIT ? OFFSET ?`).bind(perPage, offset).all();
...
export async function handleCreateHostel(request: Request, env: Env): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  const id = crypto.randomUUID();
  await env.PLATFORM_CONTEXT!.db.prepare(`INSERT INTO hostels (id,name,type,capacity,location,status) VALUES (?,?,?,?,?,?)`).bind(id, body.name, body.type||'Male', body.capacity||0, body.location||null, 'Available').run();
...
export async function handleListRoomAssignments(request: Request, env: Env): Promise<Response> {
  ...
  const where = `WHERE ${filters.join(' AND ')}`;
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM hostel_room_assignments ra ${where}`).bind(...bindings).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(
  ...
  const items = results.map((r: RoomRow) => ({
    id: r.id as string, student_id: r.student_id as string, studentName: r.student_name as string,
    hostelId: r.hostel_id as string, hostelName: r.hostel_name as string, roomNumber: r.room_number as string,
    checkInDate: r.check_in_date as string, status: r.status as string
  }));
  return json({ success: true, data: items, total, page, perPage });
}
...
export async function handleCreateRoomAssignment(request: Request, env: Env): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  const id = crypto.randomUUID();
  await env.PLATFORM_CONTEXT!.db.prepare(`INSERT INTO hostel_room_assignments (id,student_id,hostel_id,room_number,check_in_date,status) VALUES (?,?,?,?,?,?)`).bind(id, body.student_id, body.hostelId||body.hostel_id, body.roomNumber||body.room_number, body.checkInDate||body.check_in_date||new Date().toISOString(), 'Active').run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM hostel_room_assignments WHERE id = ?`).bind(id).first();
  return ok(row);
}
...
export async function handleListMedicalRecords(request: Request, env: Env): Promise<Response> {
  const url = new URL(request.url);
  const { page, perPage, offset } = paginate(url);
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM medical_records`).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(
  ...
  const items = results.map((r: MedRow) => ({
    id: r.id as string, student_id: r.student_id as string, studentId: r.student_id as string,
    studentName: r.student_name as string, condition: r.condition_name as string, bloodType: r.blood_type as string,
    date: r.visit_date as string, attendingStaff: r.attending_staff as string, status: r.status as string,
    vitals: (() => { try { return JSON.parse((r.vitals as string) || '{}'); } catch { return {}; } })(),
...
export async function handleListInventory(request: Request, env: Env): Promise<Response> {
  ...
  const where = `WHERE ${filters.join(' AND ')}`;
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM inventory_items ${where}`).bind(...bindings).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM inventory_items ${where} ORDER BY name LIMIT ? OFFSET ?`).bind(...bindings, perPage, offset).all();
  return json({ success: true, data: results, total, page, perPage });
}
...
export async function handleCreateInventoryItem(request: Request, env: Env): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  const id = crypto.randomUUID();
  await env.PLATFORM_CONTEXT!.db.prepare(`INSERT INTO inventory_items (id,name,category,quantity,unit,location,status,cost_per_unit,supplier) VALUES (?,?,?,?,?,?,?,?,?)`).bind(id, body.name, body.category||null, body.quantity||0, body.unit||'pcs', body.location||null, body.status||'In Stock', body.cost_per_unit||0, body.supplier||null).run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM inventory_items WHERE id = ?`).bind(id).first();
  return ok(row);
}

export async function handleUpdateInventoryItem(request: Request, env: Env, id: string): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  await env.PLATFORM_CONTEXT!.db.prepare(`UPDATE inventory_items SET name=COALESCE(?,name), quantity=COALESCE(?,quantity), status=COALESCE(?,status), updated_at=datetime('now') WHERE id=?`).bind(body.name||null, body.quantity??null, body.status||null, id).run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM inventory_items WHERE id = ?`).bind(id).first();
  return ok(row);
}
...
export async function handleListVisitors(request: Request, env: Env): Promise<Response> {
  const url = new URL(request.url);
  const { page, perPage, offset } = paginate(url);
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM visitors`).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM visitors ORDER BY check_in DESC LIMIT ? OFFSET ?`).bind(perPage, offset).all();
  return json({ success: true, data: results, total, page, perPage });
}
...
export async function handleCreateVisitor(request: Request, env: Env): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  const id = crypto.randomUUID();
  await env.PLATFORM_CONTEXT!.db.prepare(`INSERT INTO visitors (id,full_name,phone,email,id_type,id_number,purpose,host_name,host_department,check_in,status) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).bind(id, body.full_name||body.fullName, body.phone||null, body.email||null, body.id_type||'National ID', body.id_number||null, body.purpose, body.host_name||body.hostName||null, body.host_department||body.hostDepartment||null, new Date().toISOString(), 'Checked In').run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM visitors WHERE id = ?`).bind(id).first();
  return ok(row);
}
...
export async function handleUpdateVisitor(request: Request, env: Env, id: string): Promise<Response> {
  ...
  await env.PLATFORM_CONTEXT!.db.prepare(`UPDATE visitors SET status=COALESCE(?,status), check_out=COALESCE(?,check_out) WHERE id=?`).bind(body.status||null, checkOut, id).run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM visitors WHERE id = ?`).bind(id).first();
  return ok(row);
}
...
export async function handleListAttendance(request: Request, env: Env): Promise<Response> {
  ...
  const where = `WHERE ${filters.join(' AND ')}`;
  const total = ((await env.PLATFORM_CONTEXT!.db.prepare(`SELECT COUNT(*) as c FROM attendance_records ar ${where}`).bind(...bindings).first<{c:number}>())?.c) || 0;
  const { results } = await env.PLATFORM_CONTEXT!.db.prepare(
...
export async function handleCreateAttendanceRecord(request: Request, env: Env): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  const id = crypto.randomUUID();
  await env.PLATFORM_CONTEXT!.db.prepare(`INSERT INTO attendance_records (id,student_id,course_id,term_id,date,status,remarks) VALUES (?,?,?,?,?,?,?)`).bind(id, body.student_id||body.studentId, body.course_id||body.courseId||null, body.term_id||body.termId||null, body.date||new Date().toISOString().split('T')[0], body.status||'Present', body.remarks||null).run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM attendance_records WHERE id = ?`).bind(id).first();
  return ok(row);
}

export async function handleUpdateAttendanceRecord(request: Request, env: Env, id: string): Promise<Response> {
  const body = await typedJson<Record<string, unknown>>(request);
  await env.PLATFORM_CONTEXT!.db.prepare(`UPDATE attendance_records SET status=COALESCE(?,status), remarks=COALESCE(?,remarks), updated_at=datetime('now') WHERE id=?`).bind(body.status||null, body.remarks||null, id).run();
  const row = await env.PLATFORM_CONTEXT!.db.prepare(`SELECT * FROM attendance_records WHERE id = ?`).bind(id).first();
  return ok(row);
}
