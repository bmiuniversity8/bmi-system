...
const ROUTES: Route[] = [
  ...
  { method: 'GET', path: /^\/api\/(?:v1\/)?(?:admin\/)?performance\/metrics$/, roles: ['admin'], handler: async (req, env) => handleGetPerformanceMetrics(req, env) },
  { method: 'GET', path: /^\/api\/(?:v1\/)?(?:admin\/)?performance\/analysis$/, roles: ['admin'], handler: async (req, env) => handleGetQueryAnalysis(req, env) },
  { method: 'POST', path: /^\/api\/(?:v1\/)?(?:admin\/)?performance\/maintenance$/, roles: ['admin'], handler: async (req, env) => handleRunMaintenance(req, env) },
  { method: 'GET', path: /^\/api\/(?:v1\/)?(?:admin\/)?performance\/health$/, roles: ['admin', 'staff'], handler: async (req, env) => handleGetSystemHealth(req, env) },
  { method: 'DELETE', path: /^\/api\/(?:v1\/)?(?:admin\/)?performance\/metrics$/, roles: ['admin'], handler: async (req, env) => handleClearMetrics(req, env) },
  ...
  { method: 'GET', path: /^\/api\/v1\/dashboard\/upcoming-deadlines$/, roles: ['admin', 'staff', 'student'], handler: async (req, env) => handleGetUpcomingDeadlines(req, env) },
  { method: 'POST', path: /^\/api\/v1\/transcripts\/register$/, roles: ['admin', 'staff', 'student'], handler: async (req, env) => handleRegisterTranscript(req, env) },
